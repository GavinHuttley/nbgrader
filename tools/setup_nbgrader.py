...
def get_service_repr(course, grader, port, token):
    service = {
        ...
        "command": ["jupyterhub-singleuser", f"--group=formgrade-{course}", "--debug",],
        "user": grader,
        "cwd": f"{HOME}/{grader}",
        "api_token": f"{token}",
    }
    ...
    if component != "create_assignment":
        subprocess.run(
            [
                ...
                action,
                "--user",
                "nbgrader.server_extensions.{}".format(component),
            ],
...
def add_course(args):
    ...
    port = get_next_port()
    print(
        "setting up service with token {} on port {} for course {}".format(
            admin_token, port, course
        )
    )
    print("---------------------------------------------------------")
    ...
    try:
        ...
        with subprocess.Popen(
            ["passwd", grader_account], stdin=subprocess.PIPE, encoding="utf-8"
        ) as proc:
            proc.stdin.write("{}\n".format(password))
            proc.stdin.write("{}\n".format(password))
...
def add_grader(grader, course):
    ...
    try:
        call_api("post", "users/{}".format(grader))
    except HTTPError as e:
    ...
    call_api(
        "post", "groups/formgrade-{}/users".format(course), datas={"users": [grader]}
    )
...
def add_teacher(args):
    ...
    print(
        "- Adding teacher {} to course : {}".format(args.teacher_name, args.course_name)
    )
    print("------------------------------------")
    ...
    try:
        ...
        with subprocess.Popen(
            ["passwd", teacher], stdin=subprocess.PIPE, encoding="utf-8"
        ) as proc:
            proc.stdin.write("{}\n".format(password))
            proc.stdin.write("{}\n".format(password))
...
def add_student(args):
    ...
    subprocess.run(
        ...
        env={
            "HOME": "{HOME}/{grader}".format(HOME=HOME, grader=grader),
            "USER": grader,
        },
    )
    ...
    if not already_exists:
        with subprocess.Popen(
            ["passwd", student], stdin=subprocess.PIPE, encoding="utf-8"
        ) as proc:
            proc.stdin.write("{}\n".format(password))
            proc.stdin.write("{}\n".format(password))
...
def install_all(args):
    ...
    with subprocess.Popen(
        ["passwd", JUPYTER_ADMIN], stdin=subprocess.PIPE, encoding="utf-8"
    ) as proc:
        proc.stdin.write("{}\n".format(ADMIN_PWD))
        proc.stdin.write("{}\n".format(ADMIN_PWD))
...
def import_students_stub(args):
    print(
        "Importing students to course '{}' from file : {}".format(
            args.course, args.file
        )
    )
...
def main():
    ...
    parser_install = subparsers.add_parser(
        "install", help="install jupyterhub and nbgrader from scratch"
    )
    parser_install.add_argument(
        "-s", "--systemd", action="store_true", help="also install startup script"
    )
    parser_install.set_defaults(func=install_all)

    # create the parser for the "add" command
    parser_add = subparsers.add_parser(
        "add", help="add a course, a teacher or a student"
    )
    ...
    parser_add_teacher = subparsers_add.add_parser(
        "teacher", help="add teacher to existing course"
    )
    parser_add_teacher.add_argument(
        "teacher_name", help="the username of the teacher to add"
    )
    ...
    parser_add_student = subparsers_add.add_parser(
        "student", help="add student to existing course"
    )
    ...
    parser_add_student.add_argument(
        "--first-name", help="the first name of the student to add"
    )
    parser_add_student.add_argument(
        "--last-name", help="the last name of the student to add"
    )
    ...
    parser_import = subparsers.add_parser(
        "import", help="import students to course from a file"
    )
